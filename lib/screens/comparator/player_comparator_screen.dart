...
class PlayerComparatorScreen extends StatefulWidget {
  ...
  const PlayerComparatorScreen({super.key, this.initialPlayer});
...
class _PlayerComparatorScreenState extends State<PlayerComparatorScreen> {
  ...
  void _selectPlayer(int index) async {
    final selected = await showSearch<Player?>(
      ...
      delegate: PlayerSearchDelegate(
        allPlayers,
        searchFieldLabel: _t(context, 'Pesquisar jogador', 'Search player'),
      ),
    );
    if (selected != null) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          _t(context, 'Comparador de Jogadores', 'Player Comparator'),
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            if (player1 != null && player2 != null) _buildLegend(),
          ],
        ),
      ),
    );
        ...
        child: Column(
          children: [
            Container(
              ...
              child: ClipOval(child: _playerPhotoWidget(player)),
            ),
            const SizedBox(height: 12),
            Text(
              player?.displayName ?? _t(context, 'Selecionar', 'Select'),
  ...
  Widget _playerPhotoWidget(Player? player) {
    if (player == null) {
      return const Icon(Icons.person_add, size: 40, color: Colors.white54);
    }
  ...
  SeasonStats _getPlayerStats(Player? player) {
    ...
    final profile =
        PlayerStatsSeed.forName(player.fullName) ??
...
class PlayerSearchDelegate extends SearchDelegate<Player?> {
  ...
  PlayerSearchDelegate(this.players, {super.searchFieldLabel});
      ...
      child: filtered.isEmpty
          ? Builder(
              builder: (context) => Center(
                child: Text(
                  _t(context, 'Nenhum jogador encontrado', 'No players found'),
